    ...
    return (
        <div >
            ...
            <div style={{fontWeight:'bold'}}>Position/s</div>
                <div>
            {playerPositions.slice(3,7).map(pos=><><label>{isCatcher(pos)}</label><input type='checkbox' checked={curPos.includes(pos)} onChange={handleClick} value={pos}/></>)}
            </div>
            <div>
            ...
            <div>
            {playerPositions.slice(7).map(pos=><><label>{pos}</label><input type='checkbox' checked={curPos.includes(pos)} onChange={handleClick} value={pos}/></>)}
            </div>
            </div>
        </div>
